...
pub async fn lookup(
    ...
    let Some(history) = find_related_history_for_version(conn, channel, version).await? else {
        return Ok(None);
    };
...
pub async fn find_related_history_for_version(
    conn: &mut SqliteConnection,
    channel: &str,
    version: &Version,
...
pub async fn record(tx: &mut Transaction, channel: &str, entries: &[Entry]) -> sqlx::Result<Version> {
    let prev_history = find_related_history_for_version(tx.as_mut(), channel, &Version::Volatile).await?;
