...
use crate::{
    Builder, Config, Project, Queue, RepositoryManager, State, builder, config::Size, profile, project, repository,
...
impl Manager {
    ...
    pub async fn load(config: Config, state: Arc<State>, event_sender: mpsc::Sender<Event>) -> Result<Self> {
        ...
        let repository_manager = RepositoryManager::new(config.repository_manager.clone());
