...
pub enum Transition {
    ...
    /// Task is failed
    ///
    /// Can transition from _any_ non-failure status
...
pub async fn transition(
    ...
    let is_valid_transition = match &transition {
        ...
        // Can occur from anything except if its already failed
        Transition::Failed { .. } => !matches!(task.status, Status::Failed),
    ...
    if !is_valid_transition {
        // If task is already failed, we won't mark it failed again
        if matches!(transition, Transition::Failed { .. }) {
            error!("Invalid task transition, task is already failed");

            return Ok(());
        } else {
            error!("Invalid task transition, marking as failed");
    ...
    match transition {
        ...
        Transition::Superceded { by } => {
            // Can be transitioned to from `blocked` so we must clear any blockers
        ...
        Transition::Failed { stashed_build_logs } => {
            // Block all queued tasks dependent on this
...
async fn fix_stuck_building(
    ...
    for task in building_tasks {
        ...
        if requeue {
            transition(tx, task.id, Transition::Retry, &manager.queue)
                .await
                .context("retry task")?;
        }
    }
